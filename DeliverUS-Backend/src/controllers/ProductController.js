...
const promote = async function (req, res) {
  ...
  try {
    const product = await Product.findByPk(req.params.productId) // Busca el producto por su id
    const restStatus = product.promoted
    if (restStatus === true) { // Si el producto ya está promocionado, se despromociona
      product.promoted = false
      await product.save({ transaction: t })
    } else if (restStatus === false) { // Si el producto no está promocionado, se promociona
      product.promoted = true
      await product.save({ transaction: t }) // Guarda el producto
    }
    await t.commit()
    res.json(product)
  } catch (err) {
